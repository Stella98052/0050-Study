...
## 研究收官聲明（2026-07-19 封版）

**特徵搜索定案**：在 0050 前十大股票池、5 日持有期、扣除交易成本的條件下，
涵蓋**技術（23）、估值（3）、成長（1）、產業動能（2）四維度共 39 個特徵**
的系統性搜索（診斷 A/B/C＋Holm 校正＋VG-7 真偽篩），未發現任何可重現且
通過真偽篩的預測訊號。Model v2 無啟動依據，不建。此為方法論上強的否定
結果：明確排除了可行空間，並把「哪些路徑已被證偽」封存為可復現的研究資產。

**系統定位（兩層）**：
- 上層：**誠實否定結論的量化研究**——證據鏈（VG-1~7、稽核報告、
  LESSONLEARNT）可追溯、可復現、可對外說明
- 下層：**潮汐快照技術檢核面板**——每日自動更新方法論檢核值
  （5MV/13MV/潮汐狀態/量能乖離），模型欄位誠實標註無判別力

**尚在進行**：Model v1 前瞻裁決（獨立樣本自動累積至 30，約 2026-07-30）；
分產業稽核（`--by-industry`）作為搜尋空間**完備性補查**，非為找訊號。

### 自選股資料儲存（與模型池隔離）

```
python fetch_custom.py --symbols 6669,2337              # 直接指定
python fetch_custom.py --watchlist custom_watchlist.csv --with-valuation
```

與前十大**同一條抓取/特徵管線**，產物寫 `data/custom/`（{代號}_ohlcv.csv、
{代號}_features.csv 無未來報酬欄、選配 _valuation.csv）。自選股不入模型池
（治理裁定：池變更須經母體規則）；`custom_watchlist.csv` 為凍結清單
（預先登記，防「表現好才選入」偏誤）。
